"""Teacher Dataset Distill v9 -- Reproducibility rerun of v8.

Same Semi-Oracle Debate architecture as v8 (PROMPT_G Pass-1 + PROMPT_DEBATE_G
Pass-2 on mismatches), but with two changes aimed at improving reproducibility
and accuracy vs v8:

  1. Model: google/gemini-3.1-pro-preview (v8 used google/gemini-3-pro-preview).
     The 3.1-preview snapshot is reported to achieve ~71% accuracy on this task
     vs ~31% for gemini-2.5-pro.
  2. Temperature: 0.1 (v8 used 0.2). Lower temperature reduces softmax sampling
     entropy for better reproducibility while leaving a small tolerance margin.

Rationale: v8 Pass-1 produced different verdicts than v2 on clips where both
should have been identical (same PROMPT_G, same clips, same frames). The
primary suspected causes are (a) temperature > 0 sampling, and (b) silent
updates to the 'gemini-3-pro-preview' model tag between the v2 and v8 runs.

Inputs:
  --replay_jsonl outputs/teacher_dataset_v8.jsonl  (same 18 clips)
  --frames_root  <same as v8>

Outputs:
  outputs/teacher_dataset_v9.jsonl
  outputs/teacher_dataset_v9.xlsx  (mismatch rows highlighted red)
"""
...
PROMPT_ID = "SEMI_ORACLE_DEBATE_V9"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Semi-Oracle with Debate v9 (reproducibility rerun)")
    ...
    client = OpenAI(
        ...
        default_headers={
            ...
            "X-Title": os.environ.get("OPENROUTER_APP_TITLE", "MMLM_Teacher_Distill_V9"),
        },
    )
    ...
    print("\n" + "=" * 60, flush=True)
    print("Semi-Oracle with Debate Summary  (v9 / SEMI_ORACLE_DEBATE_V9)", flush=True)
    print("=" * 60, flush=True)
    print("Pass-1 Confusion Matrix (PROMPT_G blind):", flush=True)
    print(
    ...
    if v2_acc is not None:
        ...
        print(f"  Delta (v9 p1 - v2): {p1_acc - v2_acc:+.1%}", flush=True)
    else:
