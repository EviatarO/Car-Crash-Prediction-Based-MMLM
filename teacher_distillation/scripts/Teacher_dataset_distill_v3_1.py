"""Teacher Dataset Distill v3.1 -- Replay-only with patched PROMPT_H.

Replays the exact 18 clips from teacher_dataset_v3.jsonl (including the
replacement clip 00613) using the patched PROMPT_H that adds priority rules
for closing-speed dominance over lane discipline.
...
def main() -> None:
    ...
    parser.add_argument("--replay_jsonl", required=True,
                        help="Path to teacher_dataset_v3.jsonl to replay exact same clips")
    ...
    client = OpenAI(
        ...
        default_headers={
            ...
            "X-Title": os.environ.get("OPENROUTER_APP_TITLE", "MMLM_Teacher_Distill_V3_1"),
        },
    )
    ...
    print("\nConfusion Matrix (v3.1):", flush=True)
