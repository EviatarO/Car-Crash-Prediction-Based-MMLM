"""Teacher Dataset Distill v7 -- Replay-only with PROMPT_ORACLE_DEEP_V3."""
...
from prompts.templates import PROMPT_ORACLE_DEEP_V3
...
def _extract_structured_fields(raw_response: str) -> Optional[Dict]:
    parsed_obj = _extract_json_object(raw_response)
    if not parsed_obj:
        return None

    st = parsed_obj.get("spatiotemporal_anchoring")
    km = parsed_obj.get("kinematic_metrics")
    cc = parsed_obj.get("causal_chain")
    cf = parsed_obj.get("counterfactual_analysis")
    ...
    return {
        "reconstruction_id": parsed_obj.get("reconstruction_id"),
        "agent": st.get("agent"),
        "inception_frame": _safe_int(st.get("inception_frame")),
        "entry_coordinates": st.get("entry_coordinates"),
        "looming_intensity": _safe_float(km.get("looming_intensity")),
        "convergence_vector": km.get("convergence_vector"),
        "trigger_event": cc.get("trigger_event"),
        "point_of_no_return_frame": _safe_int(cc.get("point_of_no_return_frame")),
        "alternative_scenario": cf.get("alternative_scenario"),
        "outcome_change": _normalize_yes_no(cf.get("outcome_change")),
        "avoidability_logic": cf.get("avoidability_logic"),
        "physics_justification": parsed_obj.get("physics_justification"),
    }
...
def _materialize_clips(
    ...
    for rec in replay_clips:
        ...
        gt_verdict = rec.get("gt_verdict", "YES" if target == 1 else "NO")
        gt_label = "COLLISION" if target == 1 else "SAFE EVENT"
        out.append(
            {
                "video_id": video_id,
        ...
        rows.append(
            {
                ...
                "agent": rec.get("agent"),
                "inception_frame": rec.get("inception_frame"),
                "entry_coordinates": rec.get("entry_coordinates"),
                "looming_intensity": rec.get("looming_intensity"),
                "convergence_vector": rec.get("convergence_vector"),
                "trigger_event": rec.get("trigger_event"),
                "point_of_no_return_frame": rec.get("point_of_no_return_frame"),
                "alternative_scenario": rec.get("alternative_scenario"),
                "outcome_change": rec.get("outcome_change"),
                "avoidability_logic": rec.get("avoidability_logic"),
                "physics_justification": rec.get("physics_justification"),
                "error": rec.get("error"),
                "review": "",
            }
        )
...
def _avg_looming(records: List[Dict], target_value: int) -> Optional[float]:
    values = [
        float(rec["looming_intensity"])
        for rec in records
        if rec.get("target") == target_value and rec.get("looming_intensity") is not None
    ]
    if not values:
        return None
...
def main() -> None:
    ...
    parser.add_argument("--replay_jsonl", required=True, help="Path to replay JSONL")
    ...
    client = OpenAI(
        ...
        default_headers={
            ...
            "X-Title": os.environ.get("OPENROUTER_APP_TITLE", "MMLM_Teacher_Distill_V7"),
        },
    )
    ...
    with open(output_jsonl, "w", encoding="utf-8") as fout:
        for i, clip in enumerate(clips, start=1):
            clip_prompt = PROMPT_ORACLE_DEEP_V3.replace("{gt_label}", clip["gt_label"])
            messages = _build_messages(clip_prompt, clip["image_b64s"], detail=args.detail)
            raw_response = ""
            usage: Dict = {}
            err = None
            t0 = time.time()
            ...
            parsed_obj = _extract_structured_fields(raw_response)
            record = {
                ...
                "reconstruction_id": parsed_obj.get("reconstruction_id") if parsed_obj else None,
                "agent": parsed_obj.get("agent") if parsed_obj else None,
                "inception_frame": parsed_obj.get("inception_frame") if parsed_obj else None,
                "entry_coordinates": parsed_obj.get("entry_coordinates") if parsed_obj else None,
                "looming_intensity": parsed_obj.get("looming_intensity") if parsed_obj else None,
                "convergence_vector": parsed_obj.get("convergence_vector") if parsed_obj else None,
                "trigger_event": parsed_obj.get("trigger_event") if parsed_obj else None,
                "point_of_no_return_frame": parsed_obj.get("point_of_no_return_frame") if parsed_obj else None,
                "alternative_scenario": parsed_obj.get("alternative_scenario") if parsed_obj else None,
                "outcome_change": parsed_obj.get("outcome_change") if parsed_obj else None,
                "avoidability_logic": parsed_obj.get("avoidability_logic") if parsed_obj else None,
                "physics_justification": parsed_obj.get("physics_justification") if parsed_obj else None,
            ...
            print(
                f"[{i}/{total}] video={clip['video_id']} target={clip['target']} gt_label={clip['gt_label']} "
                f"latency={latency_s:.2f}s status={status}",
                flush=True,
            )

    _write_excel(run_records, output_xlsx=output_xlsx)
    tp_avg = _avg_looming(run_records, target_value=1)
    tn_avg = _avg_looming(run_records, target_value=0)
    err_count = sum(1 for rec in run_records if rec.get("error"))
    print("\nOracle Deep Audit Summary (v7 / PROMPT_ORACLE_DEEP_V3):", flush=True)
    print(f"Errors: {err_count}/{total}", flush=True)
    print(f"Avg looming_intensity (TP/target=1): {tp_avg if tp_avg is not None else 'N/A'}", flush=True)
    print(f"Avg looming_intensity (TN/target=0): {tn_avg if tn_avg is not None else 'N/A'}", flush=True)
