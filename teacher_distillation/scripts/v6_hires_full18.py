"""Run PROMPT_G_OPT_v6_balanced @ 1280x720, detail='high' on the 12 GT clips
that were NOT included in the original 6-clip hi-res diagnostic.

Combined with outputs/prompt_bakeoff/highres_test.jsonl (6 already-tested
problem clips), this gives full 18-clip coverage at hi-res.

Resumable via outputs/prompt_bakeoff/v6_hires_full18.jsonl key=video_id.
"""
...
# The 12 clips NOT already in highres_test.jsonl.
CLIPS_TO_RUN = [
...
OUT_JSONL = REPO_ROOT / "outputs" / "prompt_bakeoff" / "v6_hires_full18.jsonl"

PROMPT_FILE = REPO_ROOT / "prompts" / "PROMPT_G_OPT_v6_balanced.py"
PROMPT_VAR = "PROMPT_G_OPT_v6_balanced"

# Safety guard: abort if a single call's prompt tokens explode beyond this.
...
def main() -> None:
    ...
    client = OpenAI(
        ...
        default_headers={
            ...
            "X-Title": os.environ.get("OPENROUTER_APP_TITLE", "MMLM_Highres18"),
        },
    )
        ...
        try:
            ...
            if prompt_tok and prompt_tok > PROMPT_TOKEN_HARD_CAP:
                raise RuntimeError(
                    ...
                    f"{PROMPT_TOKEN_HARD_CAP} — aborting to avoid runaway cost"
                )
