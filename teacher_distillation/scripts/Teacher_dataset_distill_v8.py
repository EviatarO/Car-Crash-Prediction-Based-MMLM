"""Teacher Dataset Distill v8 -- Semi-Oracle with Debate (PROMPT_G + PROMPT_DEBATE_G).

Two-pass experiment over 18 clips:
  Pass 1: PROMPT_G blind prediction (identical to v2 for fair comparison).
  Pass 2: PROMPT_DEBATE_G debate advocate call -- only for Pass-1 mismatches.
           The model argues for the GT outcome without being told it was wrong.

Fixes vs the earlier v8 draft
------------------------------
1. OpenRouter connectivity: per-exception-type retry strategy for APITimeoutError,
   RateLimitError, and APIConnectionError rather than a generic catch-all that
   surfaced connectivity hiccups as fatal errors.
2. No-prediction cases: mismatch now requires verdict_p1 is not None -- a parse
   failure (no error, but no parseable verdict) no longer triggers a wasteful
   Pass-2 call.
3. Timeout: timeouts are retried with exponential backoff up to max_retries instead
   of raising immediately on the first timeout.
"""
...
PROMPT_ID = "SEMI_ORACLE_DEBATE_V8"
...
def _call_model(
    ...
    Retry strategy (all retried up to max_retries):
      ...
      - RateLimitError      : 2× back-off to respect rate windows
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Semi-Oracle with Debate v8")
    parser.add_argument("--replay_jsonl", required=True, help="Path to replay JSONL (e.g. teacher_dataset_v3.jsonl)")
    ...
    client = OpenAI(
        ...
        default_headers={
            ...
            "X-Title": os.environ.get("OPENROUTER_APP_TITLE", "MMLM_Teacher_Distill_V8"),
        },
    )
    ...
    print("\n" + "=" * 60, flush=True)
    print("Semi-Oracle with Debate Summary  (v8 / SEMI_ORACLE_DEBATE_V8)", flush=True)
    print("=" * 60, flush=True)
    ...
    if v2_acc is not None:
        ...
        print(f"  Delta (v8 p1 - v2): {p1_acc - v2_acc:+.1%}", flush=True)
    else:
