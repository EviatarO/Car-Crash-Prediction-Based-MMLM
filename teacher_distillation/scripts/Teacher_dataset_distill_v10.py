"""Teacher Dataset Distill v10 -- Dual-Hypothesis PROMPT_G2 experiment.

Same Semi-Oracle Debate architecture as v9 (blind Pass-1 + PROMPT_DEBATE_G
Pass-2 on mismatches), but replaces PROMPT_G with PROMPT_G2 for Pass-1.

PROMPT_G2 keeps Steps 1-5 identical to PROMPT_G, but replaces the single-gate
Step 6 verdict with a dual-hypothesis structure inspired by SafeVL's
counterfactual reasoning:

  Step 6A: COLLISION HYPOTHESIS -- evidence FOR collision
  Step 6B: SAFETY HYPOTHESIS -- evidence AGAINST collision
  Step 6C: VERDICT -- which hypothesis has stronger physical evidence?

Rationale: v9 Pass-1 showed TP=9/9 but FP=7/9 (61.1% accuracy). PROMPT_G's
Step 6 uses an OR-gate with 4 collision conditions, making YES easy to trigger.
The dual-hypothesis forces the model to populate BOTH sides before deciding,
breaking the one-way YES funnel.

Model: google/gemini-3.1-pro-preview (same as v9)
Temperature: 0.1 (same as v9)

Inputs:
  --replay_jsonl outputs/teacher_dataset_v8.jsonl  (same 18 clips)
  --frames_root  <same as v9>

Outputs:
  outputs/teacher_dataset_v10.jsonl
  outputs/teacher_dataset_v10.xlsx  (mismatch rows highlighted red)
"""
...
from prompts.templates import PROMPT_DEBATE_G, PROMPT_G2
...
PROMPT_ID = "SEMI_ORACLE_DEBATE_V10"
        ...
        rows.append(
            {
                ...
                "collision_hypothesis": rec.get("collision_hypothesis"),
                "safety_hypothesis": rec.get("safety_hypothesis"),
                # Comparison
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Semi-Oracle with Debate v10 (PROMPT_G2 dual-hypothesis)")
    ...
    client = OpenAI(
        ...
        default_headers={
            ...
            "X-Title": os.environ.get("OPENROUTER_APP_TITLE", "MMLM_Teacher_Distill_V10"),
        },
    )
    ...
    with open(output_jsonl, "w", encoding="utf-8") as fout:
        ...
        for i, clip in enumerate(clips, start=1):
            ...
            # ------------------------------------------------------------------
            # Pass 1 -- blind prediction with PROMPT_G2 (dual-hypothesis)
            # ------------------------------------------------------------------
            print(
            ...
            messages_p1 = _build_messages(PROMPT_G2, clip["image_b64s"], detail=args.detail)
            raw_p1 = ""
            usage_p1: Dict = {}
            ...
            record: Dict = {
                ...
                "collision_hypothesis": parsed_p1.get("collision_hypothesis") if parsed_p1 else None,
                "safety_hypothesis": parsed_p1.get("safety_hypothesis") if parsed_p1 else None,
    ...
    print("\n" + "=" * 60, flush=True)
    print("Semi-Oracle with Debate Summary  (v10 / SEMI_ORACLE_DEBATE_V10)", flush=True)
    print("=" * 60, flush=True)
    print("Pass-1 Confusion Matrix (PROMPT_G2 dual-hypothesis):", flush=True)
    print(
    ...
    if v2_acc is not None:
        ...
        print(f"  Delta (v10 p1 - v2): {p1_acc - v2_acc:+.1%}", flush=True)
    else:
